...
class MeasurementValidation(NamedTuple):
    ...
    def as_pytest_param(self):
        return pytest.param(
            ...
            self.compass_trend,
            self.error_margin,
            self.gamma_error_margin,
            id=self.id,
            marks=() if not self.xfail else (pytest.mark.xfail,),
        )


create_measurement_validation_lope = partial(
    MeasurementValidation, id="lope_202510_measurement"
)
...
create_measurement_validation_synt_2025 = partial(
    MeasurementValidation, id="synt_2025_measurement"
)

measurement_validations_synt_2025 = (
    create_measurement_validation_synt_2025(
...
create_measurement_validation_synt_2025_v2 = partial(
    MeasurementValidation, id="measurement_validations_synt_2025_v2"
)
...
@pytest.mark.parametrize(
    ...
    [
        *map(
            lambda measurement_validation: measurement_validation.as_pytest_param,
            itertools.chain(
                measurement_validations_lope_202510,
                measurement_validations_synt_2025,
                measurement_validations_synt_2025_v2,
