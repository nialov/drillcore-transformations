...
_ALPHA, _BETA, _GAMMA, _MEASUREMENT_DEPTH, _DEPTH, _BOREHOLE_TREND, _BOREHOLE_PLUNGE = base_measurements
...
def transform(measurementfile, depthfile=None, outputfile=None, gamma=False):
	...
	if depthfile is not None:
		...
		if ".csv" in measurementfile:
			# csv. files
			click.echo(click.style("Transforming with csv measurement and depth files.", fg="cyan"))
			usage.transform_csv_two_files(measurementfile, depthfile, gamma, outputfile)
		elif ".xls" in measurementfile:
			# excel files
			click.echo(click.style("Transforming with excel measurement and depth files.", fg="cyan"))
		...
		if ".csv" in measurementfile:
			# csv. file
			click.echo(click.style("Transforming with csv measurement file.", fg="cyan"))
			usage.transform_csv(measurementfile, gamma, outputfile)
		elif ".xls" in measurementfile:
			# excel file
			click.echo(click.style("Transforming with excel measurement file.", fg="cyan"))
...
@cli.command()
@click.option("--initialize", "-i", is_flag=True, help="WARNING: Initializes a new, default, config.ini when run.")
...
def conventions():
	...
	"""
	click.echo(click.style("Changing conventions through command line not supported.", fg="red"))
	click.echo(click.style("Instead, you should edit config.ini through the method change_conventions in usage.py "
						   "or manually with a text editor.", fg="yellow"))
	c = usage.find_config()
	click.echo(click.style(f"Config File Path: \n{c}", fg="green"))
